...
// https://code.google.com/archive/p/smallsha1
...
inline unsigned int rol(unsigned int value, unsigned int steps) { return ((value << steps) | (value >> (32 - steps))); }
    ...
    while (currentBlock <= endOfFullBlocks) {
      ...
      for (int roundPos = 0; currentBlock < endCurrentBlock; currentBlock += 4) {
        ...
        w[roundPos++] = (unsigned int)sarray[currentBlock + 3] | (((unsigned int)sarray[currentBlock + 2]) << 8) |
                        (((unsigned int)sarray[currentBlock + 1]) << 16) | (((unsigned int)sarray[currentBlock]) << 24);
      }
      innerHash(result, w);
    }
  }
