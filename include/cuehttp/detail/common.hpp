...
// global variables
static constexpr std::string_view g_cookie_expires_date{
    "Thu, 01 Jan 1970 00:00:00 GMT"};

static constexpr std::string_view g_methods[] = {
    "DELETE", "GET", "HEAD", "POST", "PUT", "CONNECT", "OPTIONS"};

// meta utilities
template <typename _Ty>
using is_middleware =
    std::is_convertible<std::decay_t<_Ty>,
                        std::function<void(context&, std::function<void()>)>>;

template <typename _Ty>
using is_middleware_without_next =
    std::is_convertible<std::decay_t<_Ty>, std::function<void(context&)>>;

template <typename _Ty>
using is_middleware_list = std::is_same<
    std::decay_t<_Ty>,
    std::vector<std::function<void(context&, std::function<void()>)>>>;
...
struct utils final : safe_noncopyable {
  ...
  static std::multimap<std::string, std::string> parse_query(
      std::string_view querystring) {
    static const auto decode = [](const std::string& str,
                                  std::string& decoded) {
      auto it = str.cbegin();
      auto end = str.cend();
      while (it != end) {
    ...
    std::string_view::const_iterator it{querystring.begin()},
        end{querystring.end()};
    while (it != end) {
  ...
  static std::vector<std::string_view> split(
      std::string_view str, std::string_view separators) noexcept {
    std::size_t start{0};
