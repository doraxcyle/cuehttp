  ...
  void expand() {
    ...
    for (const auto& header : request.headers_) {
      headers.emplace_back(
          std::make_pair(std::string_view{buffer_.data() + (header.first.data() - data), header.first.length()},
    ...
    if (!request.method_.empty()) {
      request.method_ = std::string_view{buffer_.data() + (request.method_.data() - data), request.method_.length()};
    }
